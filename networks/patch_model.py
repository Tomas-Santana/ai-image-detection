...
class Patch5Model(nn.Module):
    ...
    def forward(self, input_img: torch.Tensor, cropped_img: torch.Tensor, scale: torch.Tensor) -> torch.Tensor:
        x = cropped_img
        batch_size, p, _, _ = x.shape #[batch_size, 3, 224, 224]
        shallow_global_maps, high_global_maps = self.clip(x) #fm[batch_size, 2048, 7, 7], whole_embedding:[batch_size, 2048]
        ...
        _, local_maps = self.clip(window_imgs.detach()) #[batchsize*self.proposalN, 2048]
