...
class ViTCOOI:  
    def __init__(self):
        self.stride: int = 16 # ViT patch size
        self.cropped_size: int = 224
        # Doubled filter sizes to account for the 14x14 grid
    ...
    def get_coordinates(self, fm: torch.Tensor, scale: torch.Tensor) -> Tuple[torch.Tensor, List[torch.Tensor]]:
        # You can paste the exact same internal implementation of get_coordinates 
        # from the original COOI class here. The math holds perfectly.
        with torch.no_grad():
            batch_size, _, fm_height, fm_width = fm.size()
            scale_min = torch.min(scale, dim=1, keepdim=True)[0].long()
            scale_base = (scale - scale_min).long() // 2
