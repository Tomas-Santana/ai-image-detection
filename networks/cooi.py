...
class ViTCOOI:  
    def __init__(self):
        self.stride: int = 16 
        self.cropped_size: int = 224
        self.score_filter_size_list: List[List[int]] = [[6, 6], [4, 4]]
        self.score_filter_num_list: List[int] = [3, 3]
        self.score_nms_size_list: List[List[int]] = [[5, 5], [5, 5]]
        self.score_nms_padding_list: List[List[int]] = [[2, 2], [2, 2]]
        self.score_corresponding_patch_size_list: List[List[int]] = [
            [224, 224],
            [112, 112],
        ]
    ...
    def get_coordinates(self, fm: torch.Tensor, scale: torch.Tensor) -> Tuple[torch.Tensor, List[torch.Tensor]]:
        with torch.no_grad():
            batch_size, _, fm_height, fm_width = fm.size()
            scale_min = torch.min(scale, dim=1, keepdim=True)[0].long()
            scale_base = (scale - scale_min).long() // 2
            ...
            for type_no in range(self.score_filter_type_size):
                ...
                for filter_no in range(self.score_filter_num_list[type_no]):
                    ...
                    # 1. SSP FIX: Find the MINIMUM activation (simple texture) instead of max
                    value_min, loc_min_flat = torch.min(score_sum_flat, dim=1)
                    loc_min = torch.stack((loc_min_flat // score_width, loc_min_flat % score_width), dim=1)
                    fps_loc_list.append(loc_min)
                    
                    # 2. SSP FIX: "Non-Minimum Suppression" 
                    # PyTorch doesn't have min_pool2d, so we use -max_pool2d(-x) to find local minimums
                    bottom_patch = -nn.functional.max_pool2d(
                        -score_sum, self.score_nms_size_list[type_no],
                        stride=1, padding=self.score_nms_padding_list[type_no],
                    )
                    value_min = value_min.view(-1, 1, 1, 1)
                    
                    # Create a mask covering the window around the selected minimum
                    erase_mask = (bottom_patch == value_min)
                    
                    # Fill that window with infinity so it is never chosen as a minimum again
                    score_sum = score_sum.masked_fill(erase_mask, float('inf'))

                    # 3. Calculate bounding boxes based on loc_min
                    loc_rate_h = (2 * loc_min[:, 0] + fm_height - score_height + 1) / (2 * fm_height)
                    loc_rate_w = (2 * loc_min[:, 1] + fm_width - score_width + 1) / (2 * fm_width)
                    loc_rate = torch.stack((loc_rate_h, loc_rate_w), dim=1)
                    loc_center = (scale_base + scale_min * loc_rate).long()
            ...
            input_loc_tensor = torch.stack(tensors=input_loc_list, dim=1)  
            return input_loc_tensor, fps_loc_list