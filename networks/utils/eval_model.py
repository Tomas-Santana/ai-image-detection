        ...
        for i, data in enumerate(tqdm(testloader)):
            ...
            if set == "CUB":
                ...
                resized_boxes = torch.cat(
                    [
                        (boxes_coor[:, 0] * scale[:, 0]).unsqueeze(1),
                        (boxes_coor[:, 1] * scale[:, 1]).unsqueeze(1),
                        (boxes_coor[:, 2] * scale[:, 0]).unsqueeze(1),
                        (boxes_coor[:, 3] * scale[:, 1]).unsqueeze(1),
                    ],
                    dim=1,
                )
                resized_coor = (
                    torch.cat(
                        [
                            resized_boxes[:, 0].unsqueeze(1),
                            resized_boxes[:, 1].unsqueeze(1),
            ...
            if i == 0:
                ...
                with SummaryWriter(
                    ...
                    for j, indice_ndarray in enumerate(indices_ndarray):
                        img = image_with_boxes(
                            local_imgs[j], coordinates_cat[indice_ndarray]
                        )
                        cat_imgs.append(img)
                    cat_imgs = np.concatenate(cat_imgs, axis=1)
                    writer.add_images(
                        status + "/" + "object image with windows",
                        cat_imgs,
                        epoch,
                        dataformats="HWC",
                    )
